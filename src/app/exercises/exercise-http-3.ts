...
const call1 = HttpSimulator.httpGet('url-1')
  .then(data => this.doLog('Elapsed: ' + (Date.now() - start) + 'ms - ' + data));
this.doLog('httpGet(url-2) running...');
const call2 = HttpSimulator.httpGet('url-2')
  .then(data => this.doLog('Elapsed: ' + (Date.now() - start) + 'ms - ' + data));
this.doLog('httpGet(url-3) running...');
const call3 = HttpSimulator.httpGet('url-3')
  .then(data => this.doLog('Elapsed: ' + (Date.now() - start) + 'ms - ' + data));
...
==> And we process the data when it has arrived!
`;
  }
  ...
  public async run() {
    const start = Date.now();
    this.doLog('httpGet(url-1) running...');
    const call1 = HttpSimulator.httpGet('url-1').then(data =>
      this.doLog('Elapsed: ' + (Date.now() - start) + 'ms - ' + data)
    );
    this.doLog('httpGet(url-2) running...');
    const call2 = HttpSimulator.httpGet('url-2').then(data =>
      this.doLog('Elapsed: ' + (Date.now() - start) + 'ms - ' + data)
    );
    this.doLog('httpGet(url-3) running...');
    const call3 = HttpSimulator.httpGet('url-3').then(data =>
      this.doLog('Elapsed: ' + (Date.now() - start) + 'ms - ' + data)
    );
    await Promise.all([call1, call2, call3]);
    this.doLog('processing received data');
